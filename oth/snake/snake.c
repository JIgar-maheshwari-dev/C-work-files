...
void init() {
    ...
    frameBuffer = (char **) malloc(WIDTH * sizeof(char *));
    if (frameBuffer == NULL) {
    ...
    int i;
    for (i = 0; i < WIDTH; i++) {
        frameBuffer[i] = (char *) malloc(HEIGHT * sizeof(char));
        if (frameBuffer[i] == NULL) {
            printf("Memory allocation failed.\n");
            exit(1);
        }
        memset(frameBuffer[i], WALL_CHAR, HEIGHT * sizeof(char));
    }

    snake = (vec2d *) malloc(WIDTH * HEIGHT * sizeof(vec2d));
    if (snake == NULL) {
        printf("Memory allocation failed.\n");
        exit(1);
    }
